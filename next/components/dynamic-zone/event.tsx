...
import { StrapiImage, getStrapiMedia } from '../ui/strapi-image';
            ...
            <Link href={`/events/${events[0].slug}`}>
              ...
              <div className="absolute bottom-6 left-10 right-6 text-white">
                <span className="bg-[#F4F4F4] text-black px-2 py-1 rounded-3xl text-xs">
                  {isAfter(new Date(), events[0].start_datetime)
                    ? 'Past Event'
                    : 'Upcoming'}
                </span>
                ...
                events[0].report ? (
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      setOpenRegistrationForm(events[0].documentId);
                    }}
                  >
        ...
        <div className="flex flex-col gap-10">
          {events.slice(1).map((event) => {
            ...
            return (
              <div
                key={event.id}
                className="flex gap-4 bg-white px-4 py-3 rounded-xl"
              >
