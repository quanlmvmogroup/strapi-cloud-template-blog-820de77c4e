...
import { Media, Toolkits } from '@/types/types';
...
export const ToolkitsRows = ({
  ...
  const searcher = new FuzzySearch(toolkits, ['title'], {
    caseSensitive: false,
  });

  const [results, setResults] = useState(toolkits);

  console.log(
    '🚀 ~ toolkits-post-row.tsx:39 ~ ToolkitsRows ~ toolkits:',
    toolkits
  );
        ...
        {results.length === 0 ? (
          ...
          <>
            <Table className="rounded-lg mt-4 w-full">
              ...
              <TableBody>
                {results.map((toolkit) => (
                  <TableRow key={toolkit.documentId}>
                    ...
                    <TableCell>
                      <div className="flex gap-1 items-center">
                        <img
                          src={`/mine-types/${MineType[toolkit.attachment.mime]?.icon}`}
                          alt={MineType[toolkit.attachment.mime]?.text}
                          width={16}
                          height={16}
                        />
                        <span>{MineType[toolkit.attachment.mime]?.text}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      {formatDate(toolkit.createdAt, 'dd MMM yyyy')}
                    </TableCell>
                    <TableCell>
                      <a
                        href={String(getStrapiMedia(toolkit.attachment.url))}
                        target="_blank"
                        rel="noreferrer"
                      >
                    ...
                    <TableCell>
                      <button
                        ...
                        onClick={() => handleDownload(toolkit.attachment)}
                      >
